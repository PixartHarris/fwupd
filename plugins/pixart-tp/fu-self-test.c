...
static void
fu_pxi_tp_firmware_xml_func(void)
{
	g_message("Enter test 1.");
	gboolean ret;
	...
	g_autoptr(FuFirmware) firmware1 = fu_pxi_tp_firmware_new();
	g_autoptr(FuFirmware) firmware2 = fu_pxi_tp_firmware_new();
	g_autoptr(GError) error = NULL;

	/* build and write */
	filename = g_test_build_filename(G_TEST_DIST, "tests", "pxi-tp.builder.xml", NULL);
...
main(int argc, char **argv)
{
	...
	/* tests go here */
	g_test_add_func("/pixart-tp/firmware{xml}", fu_pxi_tp_firmware_xml_func);
	return g_test_run();
}
